...
def train(model, device, train_loader, optimizer, criterion, epoch, batch_size, out_text):
    ...
    out_text.write('Train set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
...
def test(model, device, test_loader, out_text):
    model.eval()
...
def run_main(FLAGS):
    ...
    plt.plot(epoch_array, train_loss1, color='r', label="Train Loss")
    plt.plot(epoch_array, test_loss1, color='g', label="Test Loss")
    plt.xlabel("EPOCH")
    plt.ylabel("LOSS")
    ...
    plt.plot(epoch_array, train_accuracy1, color='r', label="Train Accuracy")
    plt.plot(epoch_array, test_accuracy1, color='g', label="Test Accuracy")
    plt.xlabel("EPOCH")
    plt.ylabel("Accuracy")
    plt.title("MODE" + str(FLAGS.mode))
    plt.legend()
